#!/usr/bin/env python3
"""
Comprehensive Fix for GenAI Agent 3D Issues

This script addresses all known issues with the GenAI Agent 3D project:
1. RedisMessageBus ping attribute error
2. LLM service generation errors
3. API structure improvements

Run this script from the root of the GenAI Agent 3D project.
"""
...
def fix_llm_service():
    ...
    if "_generate_ollama" not in content:
        print(f"❌ Could not find _generate_ollama method in {file_path}")
        return False
    
    # Find the _generate_ollama method
    ollama_method_pattern = re.compile(r'async def _generate_ollama.*?\n    [^\s]', re.DOTALL)
    match = ollama_method_pattern.search(content)
    
    if not match:
        print(f"❌ Could not locate the _generate_ollama method in {file_path}")
        return False
    ...
    improved_method = """async def _generate_ollama(self, prompt: str, model: str, parameters: Dict[str, Any]) -> str:
    ...
    # Replace the method - need to handle the last character that was matched as part of the next section
    last_char = current_method[-1]
    modified_content = content.replace(current_method, improved_method + last_char)
...
def create_api_structure():
    ...
    api_dir = "genai_agent_project/genai_agent/api"
    ...
    llm_content = """\"\"\"API for LLM interactions in GenAI Agent 3D\"\"\"
...
def fix_llm_api_routes():
    ...
    if not os.path.exists(file_path):
        print(f"❌ LLM API routes file not found at {file_path}")
        return False
...
if __name__ == "__main__":
    separator("GenAI Agent 3D - Comprehensive Fixes")
    
    # Get current directory
    cwd = os.getcwd()
    if not os.path.exists("genai_agent_project"):
        print(f"❌ Error: This script should be run from the root of the GenAI Agent 3D project")
        print(f"Current directory: {cwd}")
        print(f"Please change to the directory containing 'genai_agent_project' folder")
        sys.exit(1)
    
    # Track overall success
    success = True
    
    separator("1. Fixing RedisMessageBus Ping")
    if not fix_redis_ping():
        success = False
    
    separator("2. Fixing LLM Service")
    if not fix_llm_service():
        success = False
    
    separator("3. Creating API Structure")
    if not create_api_structure():
        success = False
    
    separator("4. Updating LLM API Routes")
    if not fix_llm_api_routes():
        success = False
    
    if success:
        separator("All Fixes Applied Successfully!")
        ...
        separator("Some Fixes Could Not Be Applied")
