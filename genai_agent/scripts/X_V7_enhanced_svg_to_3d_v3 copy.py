...
class SVGTo3DConverter:
    ...
    def create_3d_line(self, element):
        ...
        try:
            ...
            thickness = stroke_width * self.scale_factor * 0.5  # Change 0.25 to 0.5 for visibility
    ...
    def create_3d_polygon(self, element):
        ...
        try:
            ...
            if not all(isinstance(p, tuple) and len(p) == 2 for p in points):
                log(f"Invalid polygon points structure: {points}")
                return None
    ...
    def create_3d_text(self, element):
        ...
        try:
            ...
            # In the create_3d_text function - add this at the beginning
            if 'x' not in element or 'y' not in element:
                log("Text element missing coordinates, skipping")
                return None
        ...
        try:
            ...
            for area in bpy.context.screen.areas:
                if area.type == 'VIEW_3D':
                    ...
                    # Reset view to show all objects - THIS IS THE PROBLEMATIC PART
                    # Override context must include both area and region
                    for region in area.regions:
                        if region.type == 'WINDOW':
                            override = {'area': area, 'region': region, 'window': bpy.context.window}
    ...
    """)
            ...
            if len(scene_objects) > 0:
                try:
                    # First set the text as active
                    for area in bpy.context.screen.areas:
                        if area.type == 'TEXT_EDITOR':
                            for region in area.regions:
                                if region.type == 'WINDOW':
                                    override = {'area': area, 'region': region, 'window': bpy.context.window, 'text': bpy.data.texts["startup.py"]}
                                    bpy.ops.text.run_script(override)
                                    log("Ran startup script")
                                    break
                            break
                    else:
                        # If no text editor is found, we can't run the script but that's ok
                        log("No text editor available to run startup script, skipping")
                except Exception as e:
...
def main():
    ...
    if len(argv) >= 2:
        ...
        # extrude_depth = 0.1
        # scale_factor = 0.01
        extrude_depth = 0.3  # Increase from 0.2 to 0.3
        scale_factor = 0.05  # Increase from 0.03 to 0.05
        debug = False
