"""
Basic color test with module reloading to ensure changes take effect
"""
...
def reload_modules():
    """Force reload of our modules to ensure changes take effect"""
    import svg_utils
    ...
    importlib.reload(svg_parser_elements)
    importlib.reload(svg_parser_paths)
    importlib.reload(svg_parser)
    importlib.reload(svg_converter_materials_final)
    importlib.reload(svg_converter_create)
    importlib.reload(svg_converter_path)
    importlib.reload(svg_converter_group)
    importlib.reload(svg_converter_scene)
    importlib.reload(svg_converter)
    importlib.reload(svg_to_3d_converter_new)
...
def test_basic_colors():
    ...
    try:
        log("=" * 50)
        log("Basic Color Test (with module reloading)")
        log("=" * 50)
                ...
                if hasattr(obj.data, 'materials'):
                    for i, mat in enumerate(obj.data.materials):
                        if mat:
                            ...
                            # Log viewport color
                            log(f"    Viewport color: {mat.diffuse_color}")
